...
class GoodsShopClassifyAdapter<T : GoodsShopClassifyBean>(private val itemClickListener:(position:Int,itemPosition:Int)->Unit) : BaseQuickAdapter<T, BaseViewHolder>(R.layout.item_goods_shop_classify, ArrayList<T>()) {
    ...
    override fun convert(helper: BaseViewHolder, item: T) {
        ...
        itemView.line_select_bottom.visibility=if (item.subType.size>0) View.GONE else View.VISIBLE

        itemView.classifyNext.text=if (item.subType.size>0) mContext.getString(R.string.main_see_all) else ""
        itemView.classifyNext.setCompoundDrawablesRelativeWithIntrinsicBounds(null,null,if (item.subType.size<=0) ContextCompat.getDrawable(mContext,R.mipmap.icon_main_gray_next) else null,null)
        ...
        adapter.setNewData(item.subType)
