...
class GoodsCouponAdapter<T : CouponBean> : BaseQuickAdapter<T, BaseViewHolder>(R.layout.item_goods_coupon, ArrayList<T>()) {
    ...
    override fun convert(helper: BaseViewHolder, item: T) {
        ...
        itemView.tv_redPacket_info.text = String.format(mContext.getString(R.string.goods_detail_coupon_info),item.limitMoney)
        itemView.tv_redPacket_time.text =String.format(mContext.getString(R.string.goods_detail_coupon_time),item.beginDate+"-"+item.invalidDate)
    }
}
